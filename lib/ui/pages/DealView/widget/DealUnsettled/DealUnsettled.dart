...
class DealUnsettled extends StatefulWidget {

  DealUnsettled(this.orderList, this.orderStatus ,listBlock, {Key? key}): super(key: key);
  // 服务中的订单
  ...
  @override
  State<DealUnsettled> createState() => _DealedState();
}

class _DealedState extends State<DealUnsettled> with AutomaticKeepAliveClientMixin {
  /// 避免重复刷新 从缓存中获取
  @override
  ...
  onRefresh() {
    EasyLoading();
    if (widget.orderStatus == null) { return; }
    DealService.requestDealList(widget.orderStatus!).then((value) {
      EasyLoading.dismiss();
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          Visibility(
            visible: widget.orderStatus != 4,
            child: PlaceholderWidget(
              title: '待服务状态无需填写日报', 
              iconName: 'dealed_placeHolder', 
              imgWidth: 128.px, 
              imgHeight: 95.px
            )
          )
